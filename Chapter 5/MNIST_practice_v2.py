...
def inference(input_tensor, reuse=False):
    """
    构造神经网络，返回输出层Tensor
    增加reuse可重用已定义的变量，比如在训练结束后使用这些系数进行预测
    使用标准差为0.1的正态分布来初始化权重
    使用初始值0.0来初始化偏置
    :param x:
    :param reuse:
    :return:
...
def train(mnist):
    ...
    with tf.variable_scope("", reuse=True):
        regularization = regularizer(
            tf.get_variable("layer1/weights", shape=[INPUT_NODE, LAYER1_NODE])) + regularizer(
            tf.get_variable("layer2/weights", shape=[LAYER1_NODE, OUTPUT_NODE]))
